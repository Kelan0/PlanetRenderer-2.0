...
uniform int histogramBins;
uniform int channel;

out vec4 fs_colour;

void main(void) {
    vec3 colour = min(texelFetch(textureSampler, ivec2(vs_vertexPosition * textureSize), 0).rgb, 1.0);
    ...
    gl_PointSize = 1.0;
    gl_Position = vec4((bin * 2.0 - 1.0) * ((histogramBins - 1.0) / histogramBins), 0.0, 0.0, 1.0);
}